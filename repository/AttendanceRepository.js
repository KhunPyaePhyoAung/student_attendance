    ...
    return {
        ...
        createRollCall: async ({
            ...
            const insertedId = await new Promise((resolve, reject) => {
                const sql = 'INSERT INTO roll_call (date, start_time, end_time, term_id, subject_id, instructor_id, attendance_code, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
                const params = [date, start_time, end_time, term_id, subject_id, instructor_id, uuidv4(), 'CLOSED'];
                connection.query(sql, params, (error, results, fields) => {
                    if (error) {
                        if (error.code === 'ER_DUP_ENTRY') {
                            const duplicateError = new Error();
